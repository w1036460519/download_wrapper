...
namespace dw {
    ...
    void TaskStore::init_schema() {
        ...
        const char *sql =
                ...
                "  PRIMARY KEY (client_id, protocol, natural_key, file_index, seg_start)"
                ");";
    ...
    std::vector<TaskRecord> TaskStore::load_active(const std::string &client_id) {
        ...
        const char *sql =
                "SELECT client_id, protocol, natural_key, protocol, name, save_path, filename, url, info_hash, magnet_link,"
    ...
    void TaskStore::update(const TaskRecord &r) {
        ...
        sqlite3_bind_int(st, 24, static_cast<int>(r.protocol));
    ...
    void TaskStore::save_task_files(const std::string &client_id, dw_protocol_t protocol,
        ...
        const char *sql =
                ...
                " (client_id, protocol, natural_key, file_index, name, ext, size, offset, status, downloaded_bytes)"
                " VALUES (?,?,?,?,?,?,?,?,?,?);";
        sqlite3_stmt *st = nullptr;
    ...
    void TaskStore::save_segments(const std::string &client_id, dw_protocol_t protocol,
                                  const std::string &natural_key, int32_t file_index,
                                  const std::vector<dw_byte_range_t> &segments) {
        // 委托批量接口：单文件区间即只含一个元素的向量
        save_segments_batch(client_id, protocol, natural_key, {{file_index, segments}});
    }
    ...
    void TaskStore::save_segments_batch(
        ...
        if (!file_segments.empty()) {
            const char *sql =
                    "INSERT INTO file_segments (client_id, protocol, natural_key, file_index, seg_start, seg_end)"
                    " VALUES (?,?,?,?,?,?);";
            sqlite3_stmt *st = nullptr;
            if (sqlite3_prepare_v2(db_, sql, -1, &st, nullptr) != SQLITE_OK) {
                sqlite3_exec(db_, "ROLLBACK;", nullptr, nullptr, nullptr);
    ...
    void TaskStore::upsert_task_file(const std::string &client_id, dw_protocol_t protocol,
        ...
        const char *sql =
                "INSERT INTO task_files (client_id, protocol, natural_key, file_index, name, size, downloaded_bytes)"
                " VALUES (?,?,?,?,?,?,?)"
                " ON CONFLICT(client_id, protocol, natural_key, file_index) DO UPDATE SET"
    ...
    std::vector<dw_byte_range_t> TaskStore::load_segments(const std::string &client_id, dw_protocol_t protocol,
                                                          const std::string &natural_key, int32_t file_index) {
    ...
    void TaskStore::clear_segments(const std::string &client_id, dw_protocol_t protocol,
                                   const std::string &natural_key) {
        sqlite3_stmt *st = nullptr;
        if (sqlite3_prepare_v2(db_,
                               "DELETE FROM file_segments WHERE client_id=? AND key_type=? AND natural_key=?;",
