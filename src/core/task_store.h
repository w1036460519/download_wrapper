...
public:
    ...
    bool load_by_natural_key(const std::string &client_id, TaskKeyType key_type,
    ...
    void update_status(const std::string &client_id, TaskKeyType key_type,
                       const std::string &natural_key, int32_t status);
    /// 删除任务及其 resume_data / task_files / file_segments（统一按复合键）。
    void remove(const std::string &client_id, TaskKeyType key_type, const std::string &natural_key);
    /// 写入 / 覆盖断点续传数据。
    void save_resume(const std::string &client_id, TaskKeyType key_type,
                     const std::string &natural_key, const uint8_t* data, size_t size);
    /// 读取断点续传数据；不存在返回空 vector。
    std::vector<uint8_t> load_resume(const std::string &client_id, TaskKeyType key_type,
                                     const std::string &natural_key);
    /// 仅清除某任务的断点续传数据。
    void clear_resume(const std::string &client_id, TaskKeyType key_type, const std::string &natural_key);
    
    // ---- 任务文件信息 ----
    
    /// 全量重写任务节点树（先删后插，事务包裹）。
    void save_task_files(const std::string &client_id, TaskKeyType key_type,
                         const std::string &natural_key,
                         const std::vector<dw_file_info_t>& files);
    /// 加载任务节点树（按 file_index 升序）；不存在返回空 vector。
    std::vector<dw_file_info_t> load_task_files(const std::string &client_id, TaskKeyType key_type,
                                                const std::string &natural_key);
    /// 任务级 0→2 传播：将全部文件节点置为完成正常（不触碰已删除态）。
    void mark_task_files_completed(const std::string &client_id, TaskKeyType key_type,
                                   const std::string &natural_key);
    /// 单文件完成标记：将指定 (key, file_index) 的文件节点置为完成（幂等，已删除态不触碰）。
    void mark_file_completed(const std::string &client_id, TaskKeyType key_type,
                             const std::string &natural_key, int32_t file_index);
    /// 批量更新文件的已下载字节数（事务包裹）；由 snapshot_segments_locked 从 ranges 推导后调用。
    void update_downloaded_bytes(const std::string &client_id, TaskKeyType key_type,
                                 const std::string &natural_key,
    ...
    void upsert_task_file(const std::string &client_id, TaskKeyType key_type,
                          const std::string &natural_key, int32_t file_index,
    ...
    void set_play_position(const std::string &client_id, TaskKeyType key_type,
                           const std::string &natural_key, int32_t file_index, int64_t position_ms);
    /// 读取文件播放进度（毫秒）；无记录返回 0。
    int64_t get_play_position(const std::string &client_id, TaskKeyType key_type,
                              const std::string &natural_key, int32_t file_index);
    
    // ---- 已下载区间快照 ----
    
    /// 覆盖写入某文件的已下载连续区间快照（事务：先删该 file_index 旧区间再批量写；空即清空）。
    void save_segments(const std::string &client_id, TaskKeyType key_type,
                       const std::string &natural_key, int32_t file_index,
                       const std::vector<dw_byte_range_t>& segments);
    /// 批量覆盖写入多文件的已下载区间快照（单事务）；替代逐文件 save_segments 调用。
    void save_segments_batch(const std::string &client_id, TaskKeyType key_type,
                             const std::string &natural_key,
                             const std::vector<std::pair<int32_t, std::vector<dw_byte_range_t>>>& file_segments);
    /// 读取某文件的已下载区间快照（按 seg_start 升序）；不存在返回空 vector。
    std::vector<dw_byte_range_t> load_segments(const std::string &client_id, TaskKeyType key_type,
                                               const std::string &natural_key, int32_t file_index);
    /// 清除某任务的全部区间快照（error 重下用）。
    void clear_segments(const std::string &client_id, TaskKeyType key_type, const std::string &natural_key);
