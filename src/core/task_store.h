...
public:
    ...
    /// 载入排队 / 活跃任务（DOWNLOADING/QUEUED/RESOLVING/PARSED），全字段填充。
    std::vector<TaskRecord> load_active();
    /// 载入全部任务（含暂停 / 完成 / 错误），用于快照列表。
    std::vector<TaskRecord> load_all();
    /// 按 (client_id, KEY_TYPE_HTTP, url) 去重回读。命中返回 true 并填充 out。仅供 add 去重。
    bool load_by_url(const std::string& client_id, const std::string& url, TaskRecord& out);
    /// 按 (client_id, KEY_TYPE_BT, info_hash) 去重回读。命中返回 true 并填充 out。仅供 add 去重。
    bool load_by_info_hash(const std::string& client_id, const std::string& info_hash, TaskRecord& out);
    /// 按 (client_id, KEY_TYPE_LOCAL, content_root) 去重回读。本地任务去重用。
    bool load_by_content_root(const std::string& client_id, const std::string& content_root, TaskRecord& out);
    /// 按完整 TaskKey 载入全字段（含分片续传态）；供低频控制操作。命中返回 true。
    bool load_by_task_key(const TaskKey& key, TaskRecord& out);
    ...
    /// 新增任务：纯 INSERT，复合键 (client_id, key_type, natural_key) 须在 r.task_key 预填。
    void insert(TaskRecord& r);
    /// 更新既有任务：按复合键原地 UPDATE 全字段。
    void update(const TaskRecord& r);
    /// 仅更新任务状态（轻量操作，避免全字段 UPDATE）。
    void update_status(const TaskKey& key, int32_t status);
    /// 删除任务及其 resume_data / task_files / file_segments（统一按 TaskKey）。
    void remove(const TaskKey& key);
    /// 写入 / 覆盖断点续传数据（key 为 TaskKey）。
    void save_resume(const TaskKey& key, const uint8_t* data, size_t size);
    /// 读取断点续传数据（key 为 TaskKey）；不存在返回空 vector。
    std::vector<uint8_t> load_resume(const TaskKey& key);
    /// 仅清除某任务的断点续传数据（key 为 TaskKey）。
    void clear_resume(const TaskKey& key);

    // ---- 任务文件信息 ----

    /// 全量重写任务节点树（先删后插，事务包裹；key 为 TaskKey）。
    void save_task_files(const TaskKey& key,
                         const std::vector<dw_file_info_t>& files);
    /// 加载任务节点树（按 file_index 升序；key 为 TaskKey）；不存在返回空 vector。
    std::vector<dw_file_info_t> load_task_files(const TaskKey& key);
    /// 任务级 0→2 传播：将全部文件节点置为完成正常（不触碰已删除态）。
    void mark_task_files_completed(const TaskKey& key);
    /// 单文件完成标记：将指定 (key, file_index) 的文件节点置为完成（幂等，已删除态不触碰）。
    void mark_file_completed(const TaskKey& key, int32_t file_index);
    /// 批量更新文件的已下载字节数（事务包裹）；由 snapshot_segments_locked 从 ranges 推导后调用。
    void update_downloaded_bytes(const TaskKey& key,
    ...
    void upsert_task_file(const TaskKey& key, int32_t file_index,
                          int64_t downloaded_bytes, int64_t total_size);

    // ---- 边下边播缓存（已合并到 task_files.play_position_ms）----

    /// 写入 / 覆盖文件播放进度（毫秒）；直接 UPDATE task_files，key 为 TaskKey。
    ...
    void save_segments(const TaskKey& key, int32_t file_index,
                       const std::vector<dw_byte_range_t>& segments);
    /// 批量覆盖写入多文件的已下载区间快照（单事务）；替代逐文件 save_segments 调用。
    void save_segments_batch(const TaskKey& key,
    ...
    void clear_segments(const TaskKey& key);
